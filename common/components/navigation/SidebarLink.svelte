...
<div role='button' tabindex='0' class='sidebar-link sidebar-link-with-icon pointer overflow-hidden flex-shrink-0 {css} {SUPPORTS.isAndroid ? `my-auto pl-12` : `my-sm-h-auto pl-14`}' title={text}
     ...
     use:click={_click}
     on:contextmenu|preventDefault={rbClick}>
  <span class='text-nowrap d-flex align-items-center w-full h-full'>
    {#if image}
      <span class='rounded d-flex {innerCss}'>
        <img src={image} class='h-30 w-30 rounded' class:img-noselect={_modal !== modal.focused} style='height: {btnSize} !important; width: {btnSize} !important' alt='logo' />
      </span>
      <span class='text ml-20' class:font-weight-bolder={_modal === modal.focused} class:font-size-16={_modal === modal.focused}>{text}</span>
    {:else}
      {@const active = !_modal ? ((!$modal || !modal.length) && $page === _page) : _modal === modal.focused}
      <span class='rounded d-flex {innerCss}'>
